...
import (
	...
	"microservice_release/pkg/middlewares"
...
func ConfigRoutes(router *gin.Engine, service service.ReleaseServiceInterface) *gin.Engine {
	main := router.Group("union")
	{
		...
		{
			release.GET("/releasetrains", middlewares.Auth(), func(c *gin.Context) {
			...
			release.GET("/releasetrains/business/:business_id", middlewares.Auth(), func(c *gin.Context) {
