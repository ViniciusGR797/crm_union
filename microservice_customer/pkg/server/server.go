...
// Server Estrutura de dados para armazenar o servidor HTTP
type Server struct {
	// SRV_POR Porta do servidor
	SRV_PORT string
...
func Run(router *gin.Engine, server Server, service service.CustomerServiceInterface) {
	// Imprime que servidor HTTP está rodando na porta tal
	log.Print("Server is running at port: ", server.SRV_PORT)

	// Fatal Roda servidor HTTP com as rotas e a porta do servidor passadas por parâmetro (caso ser erro dá Fatal erro - fecha o sistema)
