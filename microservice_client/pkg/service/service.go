...
func (ps *Client_service) GetClientsMyGroups(ID *int) (*entity.ClientList, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&client.ID, &client.Name, &client.Email, &client.Role_ID, &client.Role, &client.Customer_ID, &client.Customer_Name, &client.Business_Name, &client.Business_ID, &client.Release_Name, &client.Release_ID, &client.User_Name, &client.Status_Description); err != nil {
