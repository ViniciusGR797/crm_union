...
import (
	...
	"microservice_user/pkg/response"
...
func GetUsers(c *gin.Context, service service.UserServiceInterface) {
	...
	if len(list.List) == 0 {
		response.Error(c, http.StatusNotFound, errors.New("no users found"))
		return
	}
...
func GetUserByID(c *gin.Context, service service.UserServiceInterface) {
	...
	if err != nil {
		response.Error(c, http.StatusBadRequest, errors.New("ID must be an integer"))
		return
	}
...
func GetUserByName(c *gin.Context, service service.UserServiceInterface) {
	...
	if len(list.List) == 0 {
		response.Error(c, http.StatusNotFound, errors.New("no user found"))
		return
	}
...
func CreateUser(c *gin.Context, service service.UserServiceInterface) {
	...
	response.Send(c, http.StatusCreated, gin.H{
		"email":    user.Email,
...
func UpdateUser(c *gin.Context, service service.UserServiceInterface) {
	...
	// Verifica se o id é zero (caso for deu erro ao editar o user no banco)
	if idResult == 0 {
		response.Error(c, http.StatusNotFound, errors.New("cannot update user"))
		return
	}
...
func Login(c *gin.Context, service service.UserServiceInterface) {
	...
	if len(user.Password) < 8 {
		response.Error(c, http.StatusBadRequest, errors.New("password too short"))
		return
	}
	...
	if hash == "" {
		response.Error(c, http.StatusUnauthorized, errors.New("incorrect credentials"))
		return
	}
	// Verifica se o user é inativo
	if user.Status != "ATIVO" {
		response.Error(c, http.StatusUnauthorized, errors.New("inactive user"))
		return
	}
	...
	response.Send(c, http.StatusOK, gin.H{
		"token": token,
	})
}
