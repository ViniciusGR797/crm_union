...
func ConfigRoutes(router *gin.Engine, service service.UserServiceInterface) *gin.Engine {
	...
	main := router.Group("union")
	{
		user := main.Group("/v1")
		{
			...
			user.GET("/users/id/:user_id", middlewares.AuthAdmin(), func(c *gin.Context) {
